const { assert, expect } = require("chai");
const { deployments, ethers, getNamedAccounts, network } = require("hardhat");

const lowSVGImageuri =
  "data:image/svg+xml;base64,";
const highSVGimageUri =
  "data:image/svg+xml;base64,";

const highTokenUri =
  "data:application/json;base64,";
const lowTokenUri =
  "data:application/json;base64,";

describe("Dynamic SVG NFT Unit Tests", function () {
  let dynamicSvgNft, deployer, aggregatorV3Mock;

  beforeEach(async function () {
    deployer = (await getNamedAccounts()).deployer;
    await deployments.fixture(["dynamicNft", "mocks"]);
    dynamicSvgNft = await ethers.getContract("DynamicSvgNft", deployer);
    aggregatorV3Mock = await ethers.getContract("MockV3Aggregator", deployer);
  });

  describe("Constructor", function () {
    it("deploys the contract", async () => {
      assert(dynamicSvgNft.address, "The contract is not deployed");
    });

    it("initializes the counter to 0", async () => {
      const counter = (await dynamicSvgNft.getTokenCounter()).toString();
      assert.equal(counter, "0", "The counter has not been initialized to 0");
    });
  });

  describe("Mint NFT", function () {
    it("updates the mapping correctly", async () => {
      const tx = await dynamicSvgNft.mintNft("300000000000");
      await tx.wait(1);
      const highValue = await dynamicSvgNft.s_tokenIdToHighValue(0);
      assert.equal(highValue, "300000000000");
    });
    it("increases the counter", async () => {
      const initialCounter = await dynamicSvgNft.getTokenCounter();
      const tx = await dynamicSvgNft.mintNft("300000000000");
      await tx.wait(1);
      const finalCounter = await dynamicSvgNft.getTokenCounter();
      expect(finalCounter).to.equal(initialCounter.add(1));
    });
    it("emits an event", async () => {
      await expect(dynamicSvgNft.mintNft("300000000000"))
        .to.emit(dynamicSvgNft, "NFTCreated")
        .withArgs("1", "300000000000");
    });
  });

  describe("Token URI", function () {
    it("reverts if id not exists", async () => {
      await expect(dynamicSvgNft.tokenURI(0)).to.be.revertedWith(
        "ERC721Metadata__URI_QueryFor_NonExistentToken"
      );
    });

    it("picks the low token uri if price is below the minted highValue", async () => {
      const tx = await dynamicSvgNft.mintNft("300000000000");
      await tx.wait(1);
      const tokenUri = await dynamicSvgNft.tokenURI(0);
      const price = await dynamicSvgNft.getPrice();
      console.log(`The mocked price of ETH/USD is ${price.toString()}`);
      assert.equal(
        tokenUri,
        lowTokenUri,
        "The token uri for the minted NFT is not the right one with the current priceFeed"
      );
    });

    it("picks the high token uri if price is above the minted highValue", async () => {
        const tx = await dynamicSvgNft.mintNft("100000000000");
        await tx.wait(1);
        const tokenUri = await dynamicSvgNft.tokenURI(0);
        const price = await dynamicSvgNft.getPrice();
        console.log(`The mocked price of ETH/USD is ${price.toString()}`);
        assert.equal(
          tokenUri,
          highTokenUri,
          "The token uri for the minted NFT is not the right one with the current priceFeed"
        );
      });
  });
});
